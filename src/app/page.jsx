...
import { Play, Pause, Square, Plus, Trash2, Clock, AlertCircle, X, Activity, Dumbbell, BarChart3, LogOut } from 'lucide-react';
  ...
  useEffect(() => {
    if (status === 'authenticated' && isDataLoaded) {
      ...
      fetch('/api/data', {
        ...
        body: JSON.stringify({ topics: tags, history })
      }).catch(err => console.error('Failed to sync user data:', err));
    }
  }, [tags, history, status, isDataLoaded]);
  ...
  if (status === 'loading' || status === 'unauthenticated' || (status === 'authenticated' && !isDataLoaded)) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-slate-500 text-sm font-medium tracking-wide animate-pulse">Syncing Cloud Workspace...</p>
      </div>
    );
  }
